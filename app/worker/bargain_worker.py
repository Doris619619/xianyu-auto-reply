...
from app.seller_chat.config import SpikeConfigError, load_spike_config
...
class BargainWorker:
    ...
    def _claim_next_id(self) -> int | None:
        ...
        with self.session_factory() as session:
            item = QueueRepository(session).claim_next_queued()
    ...
    async def _process_item(self, item_id: int) -> None:
        ...
        try:
            async with self.chat_factory.open(
                item_url=detail_url,
    ...
    async def _run_session(
        ...
        *,
        ...
        prior_seller = tuple(e.text for e in full if e.speaker == "seller")
...
def build_default_worker(session_factory: sessionmaker) -> BargainWorker:
    ...
    factory = PersistentPlaywrightChatFactory(config.settings, guard)
